...
- Keep `requireAuth()` and `formData` handling.
- Add validation: reject non-image files and files over 10MB (HTTP 400).
- Call `uploadPhoto(file)`; return `{ url, name, size, fileId }`.
- `fileId` is extra metadata; the client only consumes `name` + `url`.

### 4. Client error surfacing (in-scope bug fix)
...
- Deleting files from Drive when a photo is removed (matches current Blob behavior — removal
  only drops the array entry).
